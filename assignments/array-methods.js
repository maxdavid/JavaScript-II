...
let allCaps = [];
allCaps = runners.map((runner, index, runners) => {
...
// Problem 1: Determine how many runners of each shirt size there are
let shirtSizes = {};
runners.forEach(runner => {
  const size = runner.shirt_size
  if (!(size in shirtSizes)) shirtSizes[size] = 0;
  shirtSizes[size]++;
})
console.log(shirtSizes);

// Problem 2: Get a list of everyone's email to add to the newsletter list
let emails = runners.map((runner, index, runners) => {
  return runner.email;
})
console.log(emails);

// Problem 3: Get a list of runners who donated less than $100
